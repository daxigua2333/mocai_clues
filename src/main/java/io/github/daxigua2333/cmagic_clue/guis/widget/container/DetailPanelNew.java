package io.github.daxigua2333.cmagic_clue.guis.widget.container;
...
import io.github.daxigua2333.cmagic_clue.mixins.ScrollPanelAccessor;
...
public class DetailPanelNew extends FlexibleContainer {
    ...
    public DetailPanelNew(Minecraft mc, int width, int height, int top, int left,
        ...
        // top buttons
        int y = top - 25;
        this.applyButton = Button.builder(Component.translatable(CMagicClue.MODID + ".screen.apply"), btn -> {
            applyChange.accept(copy);
            this.setState(State.READONLY);
        }).bounds(left + width - 80, y, 40, 20).build();
        this.cancelButton = Button.builder(Component.translatable(CMagicClue.MODID + ".screen.cancel"), btn -> {
            this.setState(State.READONLY);
        }).bounds(left + width - 40, y, 40, 20).build();

        this.editButton = Button.builder(Component.translatable(CMagicClue.MODID + ".screen.edit"), btn -> {
            if (object == null) return;  // actually buttons won't be built if object is null
            this.setState(State.EDIT);
        }).bounds(left + width - 40, y, 40, 20).build();

        this.deleteButton = Button.builder(Component.translatable(CMagicClue.MODID + ".screen.delete"), btn -> {
            ...
            ConfirmScreen confirm = new ConfirmScreen(
                    ...
                    Component.translatable(CMagicClue.MODID + ".screen.confirm_delete.title"),
                    Component.translatable(CMagicClue.MODID + ".screen.confirm_delete.body"),
                    Component.translatable(CMagicClue.MODID + ".screen.delete"),
